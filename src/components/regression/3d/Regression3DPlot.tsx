...
export const Regression3DPlot: React.FC<Regression3DPlotProps> = ({
  ...
  const x1Grid = Array.from(
    { length: gridSize },
    (_, i) => x1Range[0] + (i * (x1Range[1] - x1Range[0])) / (gridSize - 1),
  );

  const x2Grid = Array.from(
    { length: gridSize },
    (_, j) => x2Range[0] + (j * (x2Range[1] - x2Range[0])) / (gridSize - 1),
  );

  const zGrid = x2Grid.map((x2) => x1Grid.map((x1) => b0 + b1 * x1 + b2 * x2));
  ...
  return (
    <Base3DPlot data={traces} layout={layoutOverrides} config={plotConfig} />
  );
};
